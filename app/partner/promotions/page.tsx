...
export default function PromotionsPage() {
  ...
  const promosListenerRef = useRef<(() => void) | null>(null);

  useEffect(() => {
    const detachPromos = () => {
      promosListenerRef.current?.();
      promosListenerRef.current = null;
    };
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        // Campaigns for this restaurant only
        promosListenerRef.current = onSnapshot(
          query(collection(db, "promotions"), where("restaurant_id", "==", rDoc.id)),
          (snap) => {
  ...
  // Campaign stats derived from the restaurant's own promotions
  const activeCount = promos.filter((p) => p.is_active && p.status === "active").length;
  const totalRedemptions = promos.reduce((sum, p) => sum + (p.used || 0), 0);
  const totalCapacity = promos.reduce((sum, p) => sum + (p.max || 0), 0);
  const redemptionRate =
    totalCapacity > 0 ? `${Math.round((totalRedemptions / totalCapacity) * 100)}%` : "—";
  ...
  return (
    <main className="flex min-h-screen bg-[#f8fafc] text-slate-900 font-sans">
      ...
      <PartnerSidebar active="promotions" />
      ...
      <section className="flex-1 flex flex-col min-w-0 overflow-y-auto">
        ...
        <header className="sticky top-0 z-30 flex items-center justify-between border-b border-slate-200 bg-white/95 backdrop-blur-md px-8 py-5">
          <div>
            ...
            <p className="text-xs font-medium text-slate-500">
              {activeCount} active campaign{activeCount === 1 ? "" : "s"} • {totalRedemptions.toLocaleString()} redemptions
            </p>
          </div>
        ...
        <div className="p-8 space-y-8">
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm flex items-center justify-between">
              <div>
                <p className="text-xs font-bold uppercase tracking-wider text-slate-400">Total Promotions</p>
                <p className="text-3xl font-black text-slate-900 mt-2">{promos.length}</p>
              </div>
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm flex items-center justify-between">
              <div>
                <p className="text-xs font-bold uppercase tracking-wider text-slate-400">Redemption Rate</p>
                <p className="text-3xl font-black text-slate-900 mt-2">{redemptionRate}</p>
              </div>
